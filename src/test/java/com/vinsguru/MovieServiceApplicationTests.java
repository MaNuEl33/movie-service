...
class MovieServiceApplicationTests {
	...
	@Test
	void allMovies() {
		final var movies = this.getMovies("/api/movies");
		Assertions.assertEquals(6, movies.size());
	}

	@Test
	void moviesByGenre() {
		final var movies = this.getMovies("/api/movies/ACTION");
	...
	private List<MovieDto> getMovies(String uri) {
		final var requestEntity = new RequestEntity<>(HttpMethod.GET, URI.create(uri));
		final var responseEntity = this.restTemplate.exchange(requestEntity, new ParameterizedTypeReference<List<MovieDto>>() {
		});
